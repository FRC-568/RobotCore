...
public class ADIS16448_IMU extends GyroBase implements Gyro, PIDSource, LiveWindowSendable {
  ...
  private double m_accel_center_x = 0.0;
  private double m_accel_center_y = 0.0;
  private double m_accel_center_z = 0.0;
  ...
  private double m_integ_pos_x = 0.0;
  private double m_integ_pos_y = 0.0;
  private double m_integ_pos_z = 0.0;
  
  private int m_accum_count2 = 0;
  private double m_accum_accel_x = 0.0;
  private double m_accum_accel_y = 0.0;
  private double m_accum_accel_z = 0.0;
  // integrated accelarometor values
  ...
  public ADIS16448_IMU() {
    ...
    String msg = String.valueOf(readRegister(kRegPROD_ID));
  ...
  public void calibrate() {
    ...
    synchronized (this) {
      ...
      m_accel_center_x = m_accum_accel_x / m_accum_count2;
      m_accel_center_y = m_accum_accel_y / m_accum_count2;
      m_accel_center_z = m_accum_accel_z / m_accum_count2;
    }
  }
  ...
  private void calculate() {
    ...
    while (!m_freed.get()) {
      ...
      synchronized (this) {
        ...
        if (Math.abs(accel_x - m_accel_center_x) > 0.01){
        	m_integ_accel_x += (accel_x - m_accel_center_x) * dt;
        	m_integ_pos_x += m_integ_accel_x * dt;
        }
        if (Math.abs(accel_y - m_accel_center_y) > 0.0075){
        	m_integ_accel_y += (accel_y - m_accel_center_y) * dt;
        	m_integ_pos_y += m_integ_accel_y * dt;
        }
        if (Math.abs(accel_z - m_accel_center_z) > 0.003){
        	m_integ_accel_z += (accel_z - m_accel_center_z) * dt;
        	m_integ_pos_z += m_integ_accel_z * dt;
        }
